...
exports.middleware = function Mount(next, app) {
    ...
    app.mount = function(spec, target) {
        ...
        target = require("../helpers").resolveApp(target);

        // add mount info to mounted app for reverse lookup
        if (!target.mountInfo) {
            target.mountInfo = [];
        }
        target.mountInfo.push({parent: app, path: spec.path, host: spec.host});
...
exports.lookup = function(target) {
    target = require("../helpers").resolveApp(target);
    if (!Array.isArray(target.mountInfo)) {
        return "";
    }
    var mounts = target.mountInfo,
        seen = [target],
        path = "";
