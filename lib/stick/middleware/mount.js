...
exports.middleware = function Mount(next, app) {
    ...
    return function mount(req) {
        ...
        req.link = function(id) {
            var spec = reverse[id];
            if (spec && spec.canonicalPath) {
                // not working with virtual hosts based mounts yet
                var path = base + spec.canonicalPath;
                return encodeURI(arguments.length > 1 ?
                        path + Array.slice(arguments, 1).join("/") : path);
            }
            return typeof parentLink === "function" ? parentLink.apply(req, arguments) : null;
        };
