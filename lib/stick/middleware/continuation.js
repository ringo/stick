 ...
 * promise that will feeds the value back to the generator once the promise is resolved.
 ...
 * This is done my calling the `continuation.activate()` before yielding the first
 * response, and adding a query parameter with name `_c` and the continuation id as value
 * to every subsequent request to be handled by this generator. When a subsequent request
 * for a generator arrives it is resumed with the new request object as value of the last 
 * yield statement.
 *
 *     function continuation(request) {
 *         var c = app.continuation.activate();
 *         while(true) {
 *             request = yield htmlResponse(linkTo(app, {_c: c}));
 *         }
 *     }
 *
 *
