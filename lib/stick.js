...
const stickware = require("./middleware");
 ...
 */
const Application = exports.Application = function Application() {

    let middleware = unhandled,
        environments = {},
        locals = new java.lang.ThreadLocal();

    function app(req) {
        const oldLocals = locals.get();
        try {
    ...
    Object.defineProperty(app, "configure", {
        value: function() {
            ...
            const components = Array.slice(arguments);
    ...
    function resolve(middleware) {
        if (typeof middleware === "string") {
            const module = stickware[middleware] || require(middleware);
    ...
    function compose(outer, inner) {
        const composed = outer(inner, app);
        ...
        if (composed.toString === functionToString) {
            const name = composed.name || outer.name || "anonymous";
            const innerName = inner.toString === functionToString ?
...
function unhandled(req) {
    const error = new Error("Unhandled request: " + req.pathInfo);
    error.notfound = true;
    throw error;
}
