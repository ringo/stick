        ...
        if (requestOrigin !== undefined) {
            ...
            if (isAllowedOrigin) {
                ...
                if (isPreflight == true) {
                    ...
                    // 7.1.5.9
                    response.headers['Access-Control-Allow-Methods'] = config.allowMethods.join(', ');
                    if (config.allowHeaders.length > 0) {
                        response.headers['Access-Control-Allow-Headers'] = config.allowHeaders.join(', ');
                    }
                } else {
                    // 6.1.4
                    if (config.exposeHeaders.length > 0) {
                        response.headers['Access-Control-Expose-Headers'] = config.exposeHeaders.join(', ');
                    }
                }
                // 6.1.3
