...
exports.middleware = function cors(next, app) {
    ...
    app.cors = function(conf) {
        for (let key in conf) {
            if (config.hasOwnProperty(key)) {
                ...
                } else if (key === "continuePreflights" && typeof(conf.continuePreflights) !== "boolean") {
                    throw new Error("continuePreflights must be boolean");
                }
    ...
    return function cors(req) {
        ...
        if (method === "OPTIONS") {
            const pref = (config.continuePreflights ? next(req) : {
                status: 204,
                headers: { "content-length": 0 },
                body: [] // fixme: check if this is working
            });
