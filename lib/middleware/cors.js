...
exports.middleware = function cors(next, app) {
    ...
    return function cors(req) {
        ...
        const isAllowedOrigin = config.allowOrigin[0] === "*" || config.allowOrigin.indexOf(requestOrigin) > -1;

        if (isAllowedOrigin) {
            const acRequestMethod = req.headers["access-control-request-method"];
            const isAllowedRequestMethod = (acRequestMethod === undefined ) ||
